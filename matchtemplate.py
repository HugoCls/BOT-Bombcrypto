...
def screen():
    ...
    cv2.imwrite(os.getcwd()+'\\data\\images\\screenshot.png', image)
   
def show_matchtemplate(img_modele,precision=0.8):
    screen()
    img_rgb = cv2.imread(os.getcwd()+'\\data\\images\\screenshot.png')
     
    # Convert it to grayscale
    img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
     
    # Read the template
    template = cv2.imread(os.getcwd()+'\\data\\images\\'+img_modele+'.png',0)
...
def matchtemplate(img_modele,precision=0.8):
    screen()
    img_rgb = cv2.imread(os.getcwd()+'\\data\\images\\screenshot.png')
...
def show_matchtemplate_cibled(img_modele,img_cible,precision):
    img_rgb = cv2.imread(os.getcwd()+'\\data\\images\\'+img_cible+'.png')
...
def width(img_name):
    img = cv2.imread(os.getcwd()+'\\data\\images\\'+img_name+'.png')
    return(img.shape[::-1])
